...
- `BakedPathGuard::__construct()` now rejects an empty string among `$extraNeedles`, throwing
  immediately instead of letting it silently fail every compile via a rejection that cannot name
  the cause.
